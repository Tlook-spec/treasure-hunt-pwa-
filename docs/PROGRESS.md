...
**阶段**：🟥 **V1 D 区 进行中**（C 区 V1-21~26 全 ✅；D 区 V1-27 多人队伍创建 ✅；UI/暖色配色改版 ✅）。下一步 V1-28 编辑端 L1 小组奖配置。

**部署**：**双分支双部署**（2026-06-22）——`release`→网站根目录（孩子玩的稳定版），`main`→`/dev/`（开发版「寻宝-DEV」+ 橙图标）。GitHub Actions 自动部署。详见 [双分支发布说明.md](双分支发布说明.md)。
**已发版**：**2026-06-25 main/release 双推**（提交 `e53ba19`）——含 V1-27 多人创建 + 本次 UI/暖色配色全套改版，CACHE_VERSION **v2.28**、BUILD_TAG `V1-27-多人+UI+暖色配色 · 2026-06-25`。孩子联网开一次 App 即可更新缓存。（上一次发版 2026-06-24 v2.26 含 V1-21~25）
...
**下一个任务**：V1-28 编辑端 L1 通关小组奖配置（groupAwardName + groupAwardImage）。V1-22 题目插图【暂缓】。
...
- [ ] V1-28 编辑端 L1 通关小组奖配置（groupAwardName + groupAwardImage 压 ≤800，可空走默认）← **下一个**
