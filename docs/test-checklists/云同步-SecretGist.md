...
- [ ] **含图片的大探险（整库 JSON > 1MB）也能正常拉取**（2026-07-09 修复：改为一律从 raw_url 取完整内容，绕开 GitHub API 对 content 字段的 1MB 截断，之前会报「不是有效的 JSON 文件」）
- [ ] **家长若曾手动建过占位 Gist（里面混着别的文件）也能拉对**（修复：优先选 `treasure-hunt-data.json`，不再盲取第一个文件）
