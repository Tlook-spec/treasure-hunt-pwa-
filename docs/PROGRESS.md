...
**阶段**：🔨 **V1 C 区 进行中**（V1-21 ✅；V1-22 暂缓；V1-23 ✅；V1-24 填空题 ✅；V1-25 随机抽题 ✅；下一步 V1-26 联调验收）。**D 区（多人）已简化为「合作式轻量版」并展开为 V1-27~V1-31 详细任务，准备在 main 开发。**
...
**已发版**：2026-06-24 把 main 全量同步到 release（含 V1-21 暂停 / V1-23 单 L1 导入 / V1-24 填空 / V1-25 随机抽题），CACHE_VERSION v2.26、BUILD_TAG `V1-25-随机抽题 · 2026-06-24`。孩子联网开一次 App 即可更新。

**下一个任务**：V1-26 C 区联调验收（手动真机三态），之后进入 D 区多人（V1-27 起，在 main 开发）。V1-22 题目插图【暂缓】。题型工作（判断/填空/随机抽题）已从 D 区前移到 C 区，D 区现在纯多人。
...
- [ ] V1-26 C 区联调 + 真机三态验收 ← **下一个（手动真机）**
### 🟥 D 区 多人（合作式轻量版，V1-27 ~ V1-31）已展开详细任务，待开发（在 main 开发不影响孩子 release）
...
- [x] V1-27 ✅ 游戏端开局多人创建（人数 1-6 + 头像可选/名字/数字编号/6 色选一，无头像用「颜色块+编号」圆实时更新；三阶段状态机；头像进 photos 表 type:'avatar'；单人零改动；SW v2.27）← **刚完成，待推送**
- [ ] V1-28 编辑端 L1 通关小组奖配置（groupAwardName + groupAwardImage 压 ≤800，可空走默认）
- [ ] V1-29 游戏端多人版通关庆典 + 小组颁奖（队名+全体头像；撒花后整队奖揭晓；单人维持现状；不改海报）
- [ ] V1-30 数据落地 + 续玩/导入导出兼容回归（players 容错、L1 奖随导出、头像不进 admin 导出）
- [ ] V1-31 D 区联调 + 真机三态验收
