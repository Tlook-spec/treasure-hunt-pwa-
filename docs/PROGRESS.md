...
**阶段**：🔨 **V1 C 区 进行中**（V1-21 ✅；V1-22 暂缓；V1-23 ✅；V1-24 填空题 ✅；V1-25 随机抽题 🔨；下一步 V1-26 联调验收）
...
**下一个任务**：V1-25（题库随机抽题，纯编辑端），之后 V1-26 C 区联调验收。V1-22 题目插图【暂缓】。题型工作（判断/填空/随机抽题）已从 D 区前移到 C 区，D 区现在纯多人。
...
- [ ] V1-25 题库随机抽题（选题面板加「🎲 随机抽 N 道」，按学科/难度/年龄/使用次数+搜索筛选当范围，优先抽实时引用数<2 软避让，加入勾选待人工确认；顺手给选题面板补「年龄」「使用次数」筛选 + 把使用次数/已用标签改成实时引用数，修正 usedCount 永远是 0 的遗留问题；纯编辑端不升 SW）← **进行中**
- [ ] V1-26 C 区联调 + 真机三态验收
### 🟥 D 区 多人（V1-27 ~ V1-34）待定（玩过 A/B/C 再确认；题型工作已前移 C 区，D 区纯多人）
