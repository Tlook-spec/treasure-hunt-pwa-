...
12. **小组颁奖(V1)**:通关给**整队**颁一枚奖(奖名+奖图每个 L1 自配,见 Level.groupAwardName/groupAwardImage,可空走内置默认),人人有份故**无保底奖**。**2026-06-27 起单人通关也显示同一枚奖**(文案改「🎉 你 获得【奖名】！」、无队名/无头像排;多人仍是队名+全体头像)
...
**当前阶段**：🟥 V1 D 区 进行中（V1-27~30 ✅ + UI/暖色配色改版 ✅，已 main/release 多次双推发版）→ 下一步 V1-31 D 区联调 + 真机三态验收

**已完成**：T01-T13（环境 + 设计）、M01-M30（MVP 全部）、V1-01~08（A 区单人打磨）、V1-09~20（B 区 地图+故事+海报）、V1-21~26（C 区:暂停/单 L1 导入导出/填空题/随机抽题/联调，已发版）、V1-27~30（D 区 多人队伍创建/小组奖配置/多人庆典颁奖/数据兼容）、双分支双部署基建
**本次发版还含（非编号）**：L1 封面图 + L2 提示图上传；游戏端全局换 V0 暖色系（橙/青绿/金/米底，见 §9）；地图三态图标重做（灰编号/红图钉/绿勾）；hint 站点步进条、答题进度+总分条、过关本站得分、通关 3 格+海报暖白化；清理游玩垃圾按钮；封面图裁剪位置；单人通关也颁奖；通关页布局调整；CACHE_VERSION 已推进至 v2.34（详见 PROGRESS.md）
**待处理**：V1-31（D 区联调 + 真机三态验收，D 区收官）→ E 区真机验收（V1-32~35）
