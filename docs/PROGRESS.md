...
- 2026-07-17：**首页藏宝图换成插画图片 + 两份文档更新**（v2.42，本次）：首屏原 CSS 画的圆徽章（`.emblem`）换成真实藏宝图插画 `play/assets/images/treasure-map.jpg`（Gemini 生成图，PIL 压到 640px/~100KB，加进 SW 预缓存），`index.html` 用 `<img class="treasure-map-img">`；同步更新 `docs/外观微调操作文档.md`（顶部记录补 v2.41 换肤 + v2.42 首页图、声明配色已换羊皮纸方案A旧色值过时、§0.5 首屏行、新增 §1.6 换首页图指引）和 `docs/知识交接文档-AI接手指南.md`（顶部视觉现状、§6.5 两类图区分、版本号）。测试清单沿用换肤那份。
