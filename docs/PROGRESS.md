...
### 🟦 V1-A 单人打磨（进行中）
- [x] V1A-01 ✅ 游戏端展示解析（保底通过时显示正确答案 + 可选解析，quiz.html bust 分支）
- [x] V1A-02 ✅ 通关成绩单 + 错题本（每站卡片含得分/错题/解析/保底标注，computeStationSummaries 纯函数可复用）
- [ ] V1A-03（已并入 V1A-02，留空）
- [x] V1A-04 ✅ 扫码成功音效（ding.mp3 预缓存）+ 取景框绿光 + ✅ 动画（scan.html + SW v2.6）
- [x] V1A-05 ✅ 通关音效（victory.mp3 预缓存，自动播 + iOS 首次触摸兜底，victory.html + SW v2.7）
- [ ] V1A-06 成就海报 v1（成绩 + 合照，暂不含地图）
- [ ] V1A-07 拼音标注（可切换，pinyin-pro@3.18.2）
- [ ] V1A-08 修 bug：B01 选项截断 + B02 飞行模式首启提示
- [ ] V1A-09 出题用字指南（文档）
- [ ] V1A-10 V1-A 联调 + 真机三态验收

### 🟨 V1-B 探险地图（待开始，做完 A 再展开）
### 🟥 V1-C 多人 + 完整庆典（待确认，建议玩过 A/B 再定）
### 🟩 V1-D 收尾增强（待开始）
### 🟪 V1-E 真实测试 + 修 bug（待开始）
